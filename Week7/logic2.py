...
from utils import (removeall, unique, first, argmax, probability,isnumber, issequence, Expr, expr, subexpressions)
...
print(test_kb, "test_knowledge base")
...
"""
COMP450 FALL 2017-18 WEEK-7 Logic

TASK-1: Propositional Logic
Please provide the relevant explanations/comment for what is performed in the previous line

"""
"""
kb1 = PropKB()
# A KB for propositional logic. Creates a new instance of the class.


print("Clauses:", kb1.clauses)
# Prints the sentence's clauses


kb1.tell(expr('P==>Q'))
# Add the sentence (P==> & ~P ==>R) to the KB
...
print(kb1.ask(expr('Q')))
# Returns a substitution that makes the query true, or, failing that, return False.
# Yields the empty substitution {} if KB entails query; else no results.

kb1.tell(expr('P'))
# Add the sentence (P) to the KB
...
# TASK-1B: Now, please attribute suitable meanings for these propositions: P, Q, R. And explain the whole story."""

"""
COMP450 FALL 2017-18 WEEK-7 Logic

TASK 2
A) Create a new KB object
B) Insert the expressions above 
C) Then
Ask V, ask W, ask Z, ask Y
...
mypropkb = PropKB()
mypropkb.tell(expr('P ==> Q'))
mypropkb.tell(expr('Q ==> S'))
mypropkb.tell(expr('S <=> T'))
mypropkb.tell(expr('T ==> V'))
mypropkb.tell(expr('W ==> Z'))
mypropkb.tell(expr('P'))
mypropkb.tell(expr('~Z'))
print(mypropkb.clauses)
print(mypropkb.ask(expr('V')))
print(mypropkb.ask(expr('W')))
print(mypropkb.ask(expr('Z')))
print(mypropkb.ask(expr('Y')))

#TASK-3: First-Order Logic (FOL)

myfolkb = FolKB(map(expr, ['Student(Ayse)',
                           'Student(Ali)',
                            'Watch(Ali, Udacity)',
                           'Watch(Ayse, Udacity)',
                           '(Student(x) & Watch(x, y)) ==> Knows(x, y)']))

"""myfolkb.tell(expr('Student(Ayse)'))
myfolkb.tell(expr('Watch(Udacity)'))
myfolkb.tell(expr('(Student(x) & Watch(x,y)) ==> Respond(x,y)'))"""
print(myfolkb.clauses)
print(myfolkb.ask(expr('Student(x)')))
